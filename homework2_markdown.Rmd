...
paste("LePen", pr_email_lepen, sep=":")
...
paste("Macron", pr_email_macron, sep=":")
...
the value for the maximum likelihood estimation for this term is 0. This sparsity cancels out the other MLEs, making the class probability estimate 0, even if the MLEs for the other tems are high. 
...
####Preprocessing: As we are actually dealing with the text, I will perform various preprocessing tasks. First, I will remove the encoding errors, as they will show up as words when tokenized. Then, I'll remove stopwords and punctuation, in addition to converting to lower case and stemming to account for 
...
labels, looking at term ocurrence as a fraction of all words in a particular class (i.e. uniform) should provide enough information to take into account the varying frequency of terms across documents of the same class. 
...
#Problem 2(c) - continued 
Removing smoothing - Because the test set may contain terms that were not used in the training set, those documents will be given a zero probability unless you add some sort of smoothing to account for this sparsity. 
...
#Test wordscore prediction model on DFM with no smoothing
...
#compute abs difference between two ranks
rank_sum_ws_nosmoothing <- sum(abs(amzn$Word_Rank_No_Smoothing - amzn$Act_Rank))
paste("Rank Sum for Dictionary Method:", rank_sum_dict, sep=" ")
paste("Rank Sum for Wordscore Method with Smoothing:", rank_sum_ws, sep=" ")
paste("Rank Sum for Wordscore Method with no Smoothing:", rank_sum_ws_nosmoothing, sep=" ")
...
####Here it appears that the optimium training size is around 50-70% of the original data set. When only given 10-20% of the data
the model appears to underfit, and thus has poor predicative power because of high variane when applied to test folds. Between 70-90%,we see problems at the other end of the spectrum - we have little bias (i.e. a complex model), but high variance, as the model does not generalize well too virgin data. 

With respect to kernel selection, here I predicted that the best kernel would be of the linear sort. I made this decision based off the assumption that we have data that is linearly separable, with a high ratio of features to documents. Because of this, it is unnecessary to project the data into a three dimensions. Using a radial kernel is also more expensive computationally, especially as the number of documents increases. My prediction was correct, as the linear kernel performed better and faster than the radial. 
